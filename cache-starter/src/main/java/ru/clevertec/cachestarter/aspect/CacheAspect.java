...
import ru.clevertec.cachestarter.exception.CacheAspectException;
...
public class CacheAspect {
    ...
    @Pointcut("execution(* *..service..*.findAll(..))")
    ...
    public Object cachingFindById(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        if (checkAnnotation(proceedingJoinPoint)) {
            UUID id = (UUID) proceedingJoinPoint.getArgs()[0];
    ...
    @Around("findAllServiceMethod()")
    public Object cachingFindAll(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        if (checkAnnotation(proceedingJoinPoint)) {
            if (findAllWasInvoked) {
                return cache.getAll();
            } else {
                Page<?> returnObject = (Page<?>) proceedingJoinPoint.proceed();
                returnObject.forEach(object -> {
                    try {
                        cache.put(findObjectId(object), object);
                    } catch (IllegalAccessException e) {
                        throw new CacheAspectException();
                    }
                });
                log.info(CACHE_CALLED);
    ...
    private UUID findObjectId(Object object) throws IllegalAccessException {
        UUID id = null;
